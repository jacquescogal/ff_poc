import React from 'react'

const Menu_icon = () => {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-menu-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="white" fill="none" stroke-linecap="round" stroke-linejoin="round">
     <path stroke="black" d="M0 0h24v24H0z" fill="none"></path>
     <path stroke="white" d="M4 6l16 0"></path>
     <path stroke="white" d="M4 12l16 0"></path>
     <path stroke="white" d="M4 18l16 0"></path>
</svg>
    )
}

export default Menu_icon